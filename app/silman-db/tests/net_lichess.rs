...
//! the clearly-labeled live test at the bottom (gated on SILMAN_NET_TESTS=1).

mod common;

use common::{live_tests_enabled, meta, open_temp_db, FixtureFetcher, Scripted};
use silman_db::net::lichess;
...
/// LIVE NETWORK TEST — runs only with SILMAN_NET_TESTS=1. Issues a single
...
fn live_lichess_single_game_export() {
    ...
    use silman_db::net::{FetchOutcome, Fetcher, UreqFetcher};
    use std::io::Read;
