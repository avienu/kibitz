...
use silman_db::net::{FetchOutcome, Fetcher};
...
/// True when live-network tests are enabled (SILMAN_NET_TESTS=1).
pub fn live_tests_enabled(name: &str) -> bool {
    if std::env::var("SILMAN_NET_TESTS").as_deref() == Ok("1") {
        true
    } else {
        eprintln!("skipping live network test {name}; set SILMAN_NET_TESTS=1 to run");
        false
    }
}
