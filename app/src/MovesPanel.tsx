...
interface MovesPanelProps {
  ...
  /** COACH-row evidence hover (run 10 unification): fires while the
   * pointer is over the CURRENT ply's narration, so the board lights its
   * explanation evidence through the existing hover pipeline (GameView
   * maps this to hoverSentence = COACH_HOVER_INDEX). Absent when Explain
   * is off or a preview is active — the rows still navigate. */
  onNarrationHover?: (hovering: boolean) => void;
}
...
export default function MovesPanel({
  ...
  const body = rows.map((row, i) => {
    switch (row.kind) {
      ...
      case "narration": {
        if (annotationMode === "hidden") return null;
        // First-class since run 10: click goes to the narrated ply (like
        // clicking the move); hovering the CURRENT ply's row additionally
        // lights its board evidence via onNarrationHover.
        const evidenceHover = row.ply === currentPly ? onNarrationHover : undefined;
        return (
          <div key={i} className={`mv-row mv-has-comment mode-${annotationMode}`}>
            <span className="mv-num" />
            <div className="mv-comment-span">
              <button
                className={`mv-narration mv-narration-nav${evidenceHover ? " evidence" : ""}`}
                title={
                  "click: go to this move" +
                  (evidenceHover ? " · hovering highlights this note's board evidence" : "") +
                  " · generated by Annotate, not hand-editable"
                }
                onClick={() => onSelectPly(row.ply)}
                onMouseEnter={evidenceHover ? () => evidenceHover(true) : undefined}
                onMouseLeave={evidenceHover ? () => evidenceHover(false) : undefined}
              >
