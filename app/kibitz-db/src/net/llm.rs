...
//!
//! [`AnthropicTransport`] implements kibitz-verbalize's provider-agnostic
...
//! gated behind `KIBITZ_LLM_TESTS=1` and skips silently without a key.

use kibitz_verbalize::llm::{LlmTransport, TransportError};
...
mod tests {
    ...
    /// Live round-trip: runs AT MOST once, only when KIBITZ_LLM_TESTS=1 and
    /// an API key is present; otherwise skips silently.
    #[test]
    fn live_anthropic_explain_roundtrip() {
        if std::env::var("KIBITZ_LLM_TESTS").as_deref() != Ok("1") {
            return;
        }
        let Ok(key) = std::env::var("ANTHROPIC_API_KEY") else {
            return;
        };
        use kibitz_verbalize::llm::LlmVerbalizer;
        let board = kibitz_core::cozy_chess::Board::default();
