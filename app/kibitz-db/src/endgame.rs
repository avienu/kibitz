...
//!   1. **Tablebase** — when a [`kibitz_tb::Tablebase`] is supplied and the
...
fn tb_probe(tb: &mut Tablebase, board: &Board) -> Option<(i8, u32)> {
    ...
    match tb.probe_root_board(board) {
        ...
        Ok(RootProbe::Move(m)) => Some((
            match m.wdl {
                kibitz_tb::Wdl::Loss => -2,
                kibitz_tb::Wdl::BlessedLoss => -1,
                kibitz_tb::Wdl::Draw => 0,
                kibitz_tb::Wdl::CursedWin => 1,
                kibitz_tb::Wdl::Win => 2,
            },
            m.dtz,
        )),
