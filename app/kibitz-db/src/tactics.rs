...
/// Lichess theme tag → kibitz motif kind. The motif kinds are the Debug
/// names of `kibitz_core::record::AlertKind`, which is exactly what the
/// kibitz-profile motif matrix uses as row keys, so profile rows join
