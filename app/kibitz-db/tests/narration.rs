...
fn full_game_narration_is_delta_driven_and_snapshot_stable() {
    let (_d, conn) = setup();
    let report = kibitz_db::annotate::annotate_game(&conn, 1, 100_000, 64).unwrap();
    ...
    kibitz_db::annotate::annotate_game(&conn, 1, 100_000, 64).unwrap();
    let again = kibitz_db::export::export_pgn(&conn, 1).unwrap();
...
fn narration_voice_setting_switches_the_prose() {
    ...
    assert_eq!(
        kibitz_db::narrate::narration_voice(&conn).unwrap(),
        Voice::Coach
    );
    kibitz_db::annotate::annotate_game(&conn, 1, 100_000, 64).unwrap();
    let coach = kibitz_db::narrate::narrations(&conn, 1).unwrap();

    kibitz_db::narrate::set_narration_voice(&conn, Voice::Neutral).unwrap();
    assert_eq!(
        kibitz_db::narrate::narration_voice(&conn).unwrap(),
        Voice::Neutral
    );
    kibitz_db::annotate::annotate_game(&conn, 1, 100_000, 64).unwrap();
    let neutral = kibitz_db::narrate::narrations(&conn, 1).unwrap();
    ...
    kibitz_db::narrate::set_narration_voice(&conn, Voice::Coach).unwrap();
    assert_eq!(
        kibitz_db::narrate::narration_voice(&conn).unwrap(),
        Voice::Coach
    );
    kibitz_db::annotate::annotate_game(&conn, 1, 100_000, 64).unwrap();
    assert_eq!(
        kibitz_db::narrate::narrations(&conn, 1).unwrap(),
        coach,
