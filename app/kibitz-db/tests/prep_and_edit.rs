...
fn annotation_edit_round_trip() {
    ...
    let (start, mut tokens) = kibitz_db::edit::game_tokens(&conn, 1).unwrap();
    ...
    board.play(e4);
    let c5 = kibitz_db::san::parse_san(&board, "c5").unwrap();
    ...
    kibitz_db::edit::update_game_tokens(&conn, 1, &tokens).unwrap();
    ...
    let (hits, _) = kibitz_db::query::find_fen(
        &conn,
    ...
    // grows with it.
    let (start2, mut tokens2) = kibitz_db::edit::game_tokens(&conn, 1).unwrap();
    ...
    let mainline = kibitz_db::movebin::mainline_of(&tokens2);
    for p in mainline {
        if let kibitz_db::movebin::Ply::Move(m) = p {
            b2.play(m);
        }
    }
    let bb5 = kibitz_db::san::parse_san(&b2, "Bb5").unwrap();
    tokens2.push(Token::Move(bb5));
    kibitz_db::edit::update_game_tokens(&conn, 1, &tokens2).unwrap();
    let (hits, _) = kibitz_db::query::find_fen(
        &conn,
