...
use kibitz_db::import::{SourceInfo, SourceKind};
use kibitz_db::repertoire::{
...
fn cards_are_keyed_by_the_ep_normalized_hash() {
    ...
    // `-` FEN (see kibitz_db::hash), or transposition lookups break.
    ...
    let dash: Board = "rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2"
        .parse()
        .unwrap();
    let key = kibitz_db::hash::position_hash(&dash) as i64;
    assert_eq!(
        kibitz_db::hash::position_hash(&played) as i64,
        key,
