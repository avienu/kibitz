...
use kibitz_db::annotate::fold_back;
...
fn confirmed_verdict_leads_the_comment() {
    ...
    let pgn = kibitz_db::export::export_pgn(&conn, 1)
        .unwrap()
        .replace('\n', " ");
    assert!(
...
fn refuted_verdict_drops_the_alert_from_prose() {
    ...
    let before = kibitz_db::export::export_pgn(&conn, 1).unwrap();
    assert!(
    ...
    let after = kibitz_db::export::export_pgn(&conn, 1).unwrap();
