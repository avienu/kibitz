...
| **total footprint** | `size + 2×framePad + gutter` — always size the container from this, not from `size` | same |
...
**Explain panel** — `flex: 0 0 47%` when expanded (a *definite* height — intrinsic sizing plus a `max-height` collapses the inner scroller to nothing), `flex: 0 0 auto` when collapsed, bottom border `var(--line)`. The Moves panel below it is `flex: 1 1 auto` with `min-height: 312px`, so Explain can never squeeze the notation down to a few rows. Explain is also **collapsible**: a `▾/▸` ghost button at the right of its header hides the body entirely (header and verdict pill stay), which is the correct state for stepping quickly through a game.
...
- Body: scrollable, padding `16px 18px 18px`.
  ...
  - **Summary first:** only the **first** sentence block renders; the rest sit behind a full-width ghost expander reading "▾ N more findings — evidence is already on the board". The pinned foot must stay ~40px: expander plus a single non-wrapping meta line (`white-space: nowrap`, ellipsis on the selection state) — a two-line meta row starves the prose body (`background var(--panel2)`, `1px solid var(--line)`, radius 6, `500 11.5px`). Expanding is per-position and resets on every move step. Crucially the **board overlay always shows the union of all findings**, expanded or not — collapsing hides prose, never evidence.
  ...
  - **Pane structure is three rows:** header (`flex 0 0 auto`) / scrolling prose (`flex 1 1 0`, `min-height 0`, `overflow auto`) / **pinned footer** (`flex 0 0 auto`, top border) holding the expander and the meta line. The expander must never live inside the scrolling region — the way out of a bounded panel cannot itself be below the fold.
  - Footer meta row (in the pinned footer): top border, `400 10.5px` `var(--faint)`, gap 14px, `·` separators — “Static screen · no engine spawned”, voice/source (“Coach voice · templates”), and selection state (“hover a line to isolate its evidence” / “filtered to d7”).
