...
use silman_core::record::{AlertKind, Severity, SideColor};
use silman_core::wsui::{screen, WsuiConfig};

fn run(fen: &str) -> silman_core::record::WsuiReport {
...
/// out). Source: perft position, crates/silman-core/src/perft.rs.
#[test]
...
fn pinned_defender_does_not_count() {
    ...
    let board: Board = fen.parse().unwrap();
    let pinned = silman_core::attack::pinned_pieces(&board, cozy_chess::Color::Black);
