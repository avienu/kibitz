...
//! template mode (docs/KIBITZ_ENGINE_SPEC.md, "kibitz-verbalize", LLM mode).
//!
...
//!    kibitz-core) and rendering each as the set of SAN spellings this
...
use kibitz_core::record::FeatureRecord;
...
The user message is one JSON document (a kibitz FeatureRecord) describing \
...
pub fn voice_prompt(voice: Voice) -> &'static str {
    match voice {
        Voice::Coach => {
            "Style — the coaching voice: write like a Kibitz-school coach. \
...
fn legal_san_set(fen: &str) -> BTreeSet<String> {
    use kibitz_core::cozy_chess::{Board, Piece};
...
fn piece_letter(piece: kibitz_core::cozy_chess::Piece) -> char {
    use kibitz_core::cozy_chess::Piece;
    match piece {
        Piece::Pawn => 'P',
        Piece::Knight => 'N',
        Piece::Bishop => 'B',
